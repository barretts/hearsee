...
BASE_PAN_SENSITIVITY = 6.0  # Very high base sensitivity
MOVEMENT_MULTIPLIER = 1.0   # Extreme movement range
...
def calculate_adaptive_sensitivity(landmarks, frame_width, frame_height):
    ...
    target_sensitivity = BASE_PAN_SENSITIVITY / max(face_ratio * 10, 0.1)  # Very high sensitivity
    target_sensitivity = np.clip(target_sensitivity, 3.0, 15.0)  # Very high range
...
def map_face_to_grid_position(nose_x_norm, nose_y_norm, face_angle_x, face_angle_y):
    """Map face position and angle to grid position with extreme range."""
    # Apply extreme movement multiplier
    # Combine nose position with face angle for maximum range
    ...
    # Allow extreme overflow for edge detection
    grid_x = np.clip(grid_x, -0.5, 1.5)
    grid_y = np.clip(grid_y, -0.5, 1.5)
...
def main():
    ...
    try:
        ...
        pygame.display.set_caption("HearSee - Extreme Range Face Tracking")
        clock = pygame.time.Clock()

        # Virtual color image
        color_grid = generate_color_grid(GRID_ROWS, GRID_COLS)

        print("Starting HearSee (Extreme Range Face Tracking)... Press ESC to quit")
        print("Sound volume has been reduced for safety")
        print("Maximum movement range for complete edge access!")
        ...
        while running:
            ...
            # Map face position and angle to grid with extreme range
            ...
            center_x = np.clip(center_x, 10, WINDOW_WIDTH - 10)
            center_y = np.clip(center_y, 10, WINDOW_HEIGHT - 10)
            
            # Draw crosshair with better visibility
            pygame.draw.line(screen, WHITE, (center_x - 15, center_y), (center_x + 15, center_y), 3)
            pygame.draw.line(screen, WHITE, (center_x, center_y - 15), (center_x, center_y + 15), 3)
            ...
            range_text = f"Movement Range: {MOVEMENT_MULTIPLIER:.1f}x (EXTREME)"
            range_surface = small_font.render(range_text, True, WHITE)
            screen.blit(range_surface, (10, 110))

            # Display cursor position info
            cursor_text = f"Cursor: ({grid_x}, {grid_y}) [{grid_x_norm:.2f}, {grid_y_norm:.2f}]"
            cursor_surface = small_font.render(cursor_text, True, WHITE)
            screen.blit(cursor_surface, (10, 140))

            # Add status text
            status_text = "Extreme range face tracking active" if cap is not None and face_mesh is not None else "Mouse control mode"
            ...
            # Add instructions
            instruction_text = "EXTREME RANGE - Full edge access - ESC to quit"
