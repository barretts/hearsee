...
# Advanced face tracking settings
PAN_RANGE_DEGREES = 10  # 10 degrees of panning range
SMOOTHING_FACTOR = 0.2  # Smoother movement
MIN_FACE_CONFIDENCE = 0.5
PAN_SENSITIVITY = 2.0  # Multiplier for pan sensitivity
...
def calculate_pan_from_face_angle(face_angle_x, face_angle_y):
    """Calculate stereo panning based on face angle with adjustable sensitivity."""
    ...
    # Convert to left/right volumes with smooth curve
...
def generate_spatial_mix(grid, cx, cy, pan_x=0, pan_y=0):
    ...
    # Generate main tones with spatial positioning
    ...
    # Add ambient spatial effects for neighboring cells
    neighbors = [
        (cx - 1, cy, 0.9, 0.1),    # left neighbor
        (cx + 1, cy, 0.1, 0.9),    # right neighbor
        (cx, cy - 1, 0.7, 0.7),    # above neighbor
        (cx, cy + 1, 0.7, 0.7),    # below neighbor
    ]

    for nx, ny, lv, rv in neighbors:
        b, g, r = safe_color(nx, ny)
        vol_r = r / 255.0 * 0.2  # Reduced volume for ambient effect
        vol_g = g / 255.0 * 0.2
        vol_b = b / 255.0 * 0.2

        t_r = generate_tone(FREQ_RED * 1.02, vol_r)  # Slightly higher pitch
        t_g = generate_tone(FREQ_GREEN * 1.02, vol_g)
        t_b = generate_tone(FREQ_BLUE * 1.02, vol_b)
...
def get_face_angles(landmarks):
    """Calculate precise face angles from MediaPipe landmarks."""
    # Get key facial landmarks for precise angle calculation
    ...
    # Calculate horizontal angle (left-right) with improved precision
    eye_center_x = (left_eye.x + right_eye.x) / 2
    face_center_x = (left_ear.x + right_ear.x) / 2
    angle_x = math.atan2(eye_center_x - face_center_x, 0.15)  # Adjusted depth
    
    # Calculate vertical angle (up-down) with improved precision
    nose_y = nose_tip.y
    eye_center_y = (left_eye.y + right_eye.y) / 2
    angle_y = math.atan2(nose_y - eye_center_y, 0.15)
...
def main():
    ...
    try:
        ...
        pygame.display.set_caption("HearSee - Advanced Face Tracking")
        clock = pygame.time.Clock()

        # Virtual color image
        color_grid = generate_color_grid(GRID_ROWS, GRID_COLS)

        print("Starting HearSee (Advanced Face Tracking)... Press ESC to quit")
        print("Sound volume has been reduced for safety")
        print("Move your face to pan the audio - 10° range with enhanced sensitivity")
        ...
        while running:
                ...
                try:
                    ...
                    if ret:
                        ...
                        if results.multi_face_landmarks:
                            ...
                            # Get precise face angles for panning
            ...
            # Display enhanced face angle information
            ...
            sensitivity_text = f"Sensitivity: {PAN_SENSITIVITY}x"
            sensitivity_surface = small_font.render(sensitivity_text, True, WHITE)
            screen.blit(sensitivity_surface, (10, 80))

            # Add status text
            status_text = "Advanced face tracking active" if cap is not None and face_mesh is not None else "Mouse control mode"
